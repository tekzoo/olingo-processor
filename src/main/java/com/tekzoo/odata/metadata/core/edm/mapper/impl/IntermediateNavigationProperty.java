...
import static com.tekzoo.odata.metadata.core.edm.mapper.exception.ODataJPAModelException.MessageKeys.REFERENCED_PROPERTY_NOT_FOUND;
...
import com.tekzoo.odata.metadata.core.edm.mapper.exception.ODataJPAModelIgnoreException;
...
//      Determine referential constraint
      ...
      for (final JoinColumn column : columns) {
        ...
        if (refColumnName == null || refColumnName.isEmpty() || name == null || name.isEmpty()) {
          ...
          if (implicitColumns > 1)
            throw new ODataJPAModelException(MessageKeys.NOT_SUPPORTED_NO_IMPLICIT_COLUMNS,
                getInternalName());
  ...
  private void buildNaviProperty() throws ODataJPAModelException {
    ...
    if (targetType == null)
      throw new ODataJPAModelException(MessageKeys.NAVI_PROPERTY_NOT_FOUND,
